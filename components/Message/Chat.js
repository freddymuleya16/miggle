...
function Chat({ data }) {
    ...
    return (
        <div className={`w-full  h-max p-3 flex items-center ${data.isCurrent && 'justify-end'}`}>
            ...
            <p className={`${data.gender == 'woman' ? 'bg-pink-500' : data.gender == 'man' ? 'bg-blue-800' : ''} max-w-md px-3 py-2 ${data.isCurrent ? 'rounded-s-2xl' : 'rounded-e-2xl'} rounded-t-2xl body-font font-poppins text-white  mx-3 text-lg`}>
                {data.message} {data.text}
            </p>
            {!data.isCurrent &&
                <div className="h-8 w-8 cursor-pointer  rounded-full flex items-center justify-center  " onClick={() => like(data.id)}>
